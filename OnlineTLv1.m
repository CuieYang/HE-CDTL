function [Tear_Acc,CORAL_A,ht,Tear_Accs,Tear_Acct] = OnlineTL(Cur_targetdata,Cur_test,Src_data,Src_origX, hs,ht,Options,CORAL_A)
    ...
    [Wt,Terr] = calculate_w(Tar_all_pred, Yt, Wt,Num_class,[]);  %????
    
    Src_m = length(Src_data);
    Ws = ones(Src_m,1);
    Src_all_pred = predict_base(Xt,hs);
    [Ws,Serr] = calculate_w(Src_all_pred, Yt, Ws,Num_class,[]);
    ...
    Wtt = [Wtt;Wts];
    Wtt = Wtt./repmat(sum(Wtt,1),Tar_m+Src_m,1);
    for i = 1:Tar_m
...
%         Wts = [Wts;Ws{i}];
%     end
%     Wts = Wts./repmat(sum(Wts,1),Src_m,1);
    for i = 1:Src_m
        Post_precs = Post_precs+Wtt(i+Tar_m,:).*Tar_all_pred{i}.soft;
    end
